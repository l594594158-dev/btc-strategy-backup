...
# 前向填充到5m: map_to_5m(src_ts, src_vals)
# => 5m bar 的 1h/4h/1d 映射
...
# === 混合数据源: 1h/4h/1d 指标来自合约K线 ===
df_contract_1h = pd.read_pickle('backtest_data/btc_contract_1h.pkl')
df_contract_4h = pd.read_pickle('backtest_data/btc_contract_4h.pkl')
df_contract_1d = pd.read_pickle('backtest_data/btc_contract_1d.pkl')

c_1h_ct = df_contract_1h['close'].astype(float).values
h_1h_ct = df_contract_1h['high'].astype(float).values
l_1h_ct = df_contract_1h['low'].astype(float).values
ts_1h_ct = df_contract_1h.index

c_4h_ct = df_contract_4h['close'].astype(float).values
h_4h_ct = df_contract_4h['high'].astype(float).values
l_4h_ct = df_contract_4h['low'].astype(float).values
ts_4h_ct = df_contract_4h.index

c_1d_ct = df_contract_1d['close'].astype(float).values
h_1d_ct = df_contract_1d['high'].astype(float).values
l_1d_ct = df_contract_1d['low'].astype(float).values
ts_1d_ct = df_contract_1d.index

c4h_closed, sma4h_closed, adx4h_closed = build_tf_indicators(c_4h_ct, h_4h_ct, l_4h_ct)
c1d_closed, sma1d_closed, _            = build_tf_indicators(c_1d_ct, h_1d_ct, l_1d_ct)
_, _, adx1h_closed = build_tf_indicators(c_1h_ct, h_1h_ct, l_1h_ct)

# 映射到5m (使用合约K线的timestamp)
c4h_5m = map_to_5m(ts_4h_ct, c4h_closed)
sma4h_5m = map_to_5m(ts_4h_ct, sma4h_closed)
adx4h_5m = map_to_5m(ts_4h_ct, adx4h_closed)
c1d_5m = map_to_5m(ts_1d_ct, c1d_closed)
sma1d_5m = map_to_5m(ts_1d_ct, sma1d_closed)
adx1h_5m = map_to_5m(ts_1h_ct, adx1h_closed)
