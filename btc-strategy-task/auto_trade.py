...
- 自定义止盈止损
- 开仓理由记录 + 微信通知
- v2.21: 信号1 RSI 30→20 | 信号3 1h ADX >25→<30 | 信号6 1h ADX >25→<30
...
def check_entry(data):
    ...
    # v2.21: 1h ADX < 30
    if adx1h < 30 and r4h['bullish'] and rd['bullish'] and pctb <= 0.15:
        if rsi5m >= 30 and rsi5m <= 40:
            ...
            entry_reason = (
                ...
                f"1h ADX={adx1h:.1f}<30趋势弱化 | 放量({vol_ratio:.1f}x)\n"
                f"4h ADX={adx4h:.1f}<35全局过滤通过\n"
                f"固定止盈止损(百分比)\n"
                f"入场: ${price:,.2f}\n"
                f"止损: ${sl:,.2f} (-{STOP_LOSS_PCT*100:.1f}%)\n"
    ...
    # v2.21: RSI 65~85 + 1h ADX < 30
    if not r4h['bullish'] and not rd['bullish'] and pctb > 0.85 and rsi5m >= 65 and rsi5m <= 85 and adx1h < 30:
