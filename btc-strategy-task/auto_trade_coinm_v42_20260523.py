...
SYMBOL = 'BTC/USD:BTC'         # 币本位合约交易对 (下单用)
SYMBOL_SPOT = 'BTC/USDT'       # 现货交易对 (K线用)
SYMBOL_RAW = 'BTCUSD_PERP'     # 币本位原生交易对 (dapi用)

# ========== 全局参数 ==========
CONTRACTS = 25            # 每仓 25张 (≈$2,500名义)
LEVERAGE = 50             # 50x 逐仓
...
def do_open(direction, reason):
    ...
    try:
        order = binance.create_order(
            SYMBOL, 'market', order_side, CONTRACTS,
            params={'positionSide': positionSide}
        )
        fill_price = float(order.get('average', price))
        log(f"✅ 已开仓: {direction} {CONTRACTS}张 @ ${fill_price:,.2f}")
    except Exception as e:
        log(f"❌ 开仓失败: {e}")
        return False

    # === 被动层: 挂SL/TP单 ===
    ensure_sl_tp(direction, CONTRACTS, sl_price, tp_price)
    ...
    wechat_msg = (
        ...
        f"数量: {CONTRACTS}张 | 杠杆: {LEVERAGE}x\n"
        f"开仓价: ${fill_price:,.2f}\n"
    ...
    work_log('开仓', f'{direction} {CONTRACTS}张 @ {fill_price} | SL={sl_price} TP={tp_price}')
    return True
...
def cancel_algo_orders(direction, reason=''):
    """撤销指定方向所有活动algo订单"""
    try:
        algos = binance.dapiprivate_get_openalgoorders({'symbol': SYMBOL_RAW})
        pos_side = direction
        to_cancel = [
            a for a in algos
        ...
        if to_cancel:
            ...
            for a in to_cancel:
                try:
                    binance.dapiprivate_delete_algoorder({
                        'symbol': SYMBOL_RAW,
                        'algoid': a['algoId']
                    })
                    cancelled += 1
    ...
    for pos_key, direction in [('long_pos', 'LONG'), ('short_pos', 'SHORT')]:
        ...
        if pnl_pct <= -SL_PCT * 100:
            ...
            try:
                ...
                send_wechat(
                    f"🛑 BTC v4.2 主动止损\n"
                    f"{direction} {qty}张 @ ${entry:,.2f}\n"
                    f"触发价: ${mark_price:,.1f} | pnl: {pnl_pct:+.2f}%\n"
                    f"⏰ {datetime.now().strftime('%H:%M:%S')}"
                )
                work_log('主动止损', f'{direction} {qty}张 @ {entry} | pnl={pnl_pct:+.2f}%')
                state[pos_key] = None
        ...
        # === 止盈 ===
            ...
            try:
                ...
                work_log('主动止盈', f'{direction} {qty}张 @ {entry} | pnl={pnl_pct:+.2f}%')
                state[pos_key] = None
                state[f'last_{direction.lower()}_signal'] = False
                changed = True
...
def check_close(state):
    ...
    if state.get('long_pos') and not long_alive:
        ...
        send_wechat(f"✅ BTC v4.2 平仓\nLONG {pos['qty']}张 @ ${pos['entry_price']:,.2f}\n⏰ {datetime.now().strftime('%H:%M:%S')}")
        work_log('平仓', f"LONG {pos['qty']}张 @ {pos['entry_price']}")
        state['long_pos'] = None
    ...
    if state.get('short_pos') and not short_alive:
        ...
        send_wechat(f"✅ BTC v4.2 平仓\nSHORT {pos['qty']}张 @ ${pos['entry_price']:,.2f}\n⏰ {datetime.now().strftime('%H:%M:%S')}")
        work_log('平仓', f"SHORT {pos['qty']}张 @ {pos['entry_price']}")
...
def print_status(r5, r1, r4, rd, state, signal, reason):
    ...
    pos_str = ''
    if long_pos: pos_str += f'🟢L {long_pos["qty"]}张 @ ${long_pos["entry_price"]:,.0f} '
    if short_pos: pos_str += f'🔴S {short_pos["qty"]}张 @ ${short_pos["entry_price"]:,.0f} '
...
def main():
    log("═══════════ BTC v4.2 启动 ═══════════")
    log(f"CONTRACTS={CONTRACTS} | LEV={LEVERAGE}x | TP=+{TP_PCT*100}% SL=-{SL_PCT*100}%")
