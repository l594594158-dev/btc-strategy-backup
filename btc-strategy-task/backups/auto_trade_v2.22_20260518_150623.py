...
def check_entry(data):
    ...
    if r4h['bullish'] and rd['bullish'] and pctb <= 0.15:
        if rsi5m >= 30 and rsi5m <= 40:
            ...
            entry_reason = (
                ...
                f"4h ADX={adx4h:.1f}<35全局过滤通过 | 放量({vol_ratio:.1f}x)\n"
                f"固定止盈止损(百分比)\n"
                f"入场: ${price:,.2f}\n"
                f"止损: ${sl:,.2f} (-{STOP_LOSS_PCT*100:.1f}%)\n"
                f"止盈: ${tp1:,.2f} (+{TAKE_PROFIT_PCT*100:.1f}%) 全仓一次性"
    ...
    if not r4h['bullish'] and not rd['bullish'] and pctb > 0.85 and rsi5m >= 65 and rsi5m <= 85:
...
# ========== 主循环 ==========
def main():
    log(f"🚀 BTC自动交易启动 v2.22 | {POLL_INTERVAL}秒轮询 | {LEVERAGE}x | 首仓{INITIAL_QTY}+补仓{ADDON_QTY}BTC | 上限{MAX_POSITIONS_PER_DIR}仓")
    log(f"v2.22: 信号3/6 移除1h ADX条件 | v2.21: 信号1 RSI→20")
