...
- 自定义止盈止损
- 开仓理由记录 + 微信通知
- v2.23: 信号3/6 新增1h ADX 20~35(既不太弱也不太强)
...
def check_entry(data):
    ...
    # v2.23: 1h ADX >20 且 <35
    if r4h['bullish'] and rd['bullish'] and pctb <= 0.15 and adx1h > 20 and adx1h < 35:
        if rsi5m >= 30 and rsi5m <= 40:
            ...
            entry_reason = (
                ...
                f"4h ADX={adx4h:.1f}<35 | 1h ADX={adx1h:.1f}(20~35) | 放量({vol_ratio:.1f}x)\n"
                f"固定止盈止损(百分比)\n"
                f"入场: ${price:,.2f}\n"
                f"止损: ${sl:,.2f} (-{STOP_LOSS_PCT*100:.1f}%)\n"
                f"止盈: ${tp1:,.2f} (+{TAKE_PROFIT_PCT*100:.1f}%) 全仓一次性"
    ...
    # v2.23: 1h ADX >20 且 <35
    if not r4h['bullish'] and not rd['bullish'] and pctb > 0.85 and rsi5m >= 65 and rsi5m <= 85 and adx1h > 20 and adx1h < 35:
...
# ========== 主循环 ==========
def main():
    log(f"🚀 BTC自动交易启动 v2.23 | {POLL_INTERVAL}秒轮询 | {LEVERAGE}x | 首仓{INITIAL_QTY}+补仓{ADDON_QTY}BTC | 上限{MAX_POSITIONS_PER_DIR}仓")
    log(f"v2.23: 信号3/6 1h ADX 20~35 | v2.21: 信号1 RSI→20")
