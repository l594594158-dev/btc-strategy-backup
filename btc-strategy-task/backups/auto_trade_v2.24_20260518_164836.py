...
- 自定义止盈止损
- 开仓理由记录 + 微信通知
- v2.24: 信号3 新增1h +DI>-DI(涨力>跌力) | 信号6 新增1h -DI>+DI(跌力>涨力)
- v2.20: 信号1 RSI 25→30 + 新增1h ADX<30 | 信号4 新增1h ADX<30
- v2.19: 信号6(做空B) RSI 82→65~85 + 新增1h ADX>25
- v2.18: 全局4h ADX<35统一过滤（所有6信号）
- v2.17: 补仓ADX保护 | 首仓记录1h+4h ADX | 补仓时ADX必须≤首仓ADX才允许
...
def check_entry(data):
    ...
    # ========== v2.18: 全局4h ADX < 35 统一过滤（所有6信号）==========
    if adx4h >= 35:
        observe = f"观望 | 4h ADX={adx4h:.1f}>=35 趋势过强，所有信号暂停"
        return None, observe, price, atr

    # === 做多分析：大周期空头 + 5m超卖反弹（逆势信号）===
    # v2.21: RSI<20 + 1h ADX<30
    if not r4h['bullish'] and not rd['bullish'] and pctb <= 0.15 and rsi5m < 20 and adx1h < 30:
        bb_l = r5m['bb_l']
        ...
        entry_reason = (
            ...
            f"5m %b={pctb:.3f} + RSI={rsi5m:.1f} 双超卖确认(RSI<20)\n"
            f"1h ADX={adx1h:.1f}<30趋势弱化确认 | 放量({vol_ratio:.1f}x)\n"
            f"4h ADX={adx4h:.1f}<35全局过滤通过\n"
            f"固定止盈止损(百分比)\n"
            f"入场: ${price:,.2f}\n"
    ...
    if adx1h < 25 and not r4h['bullish'] and not rd['bullish'] and pctb <= 0.15 and rsi5m <= 30:
        ...
        entry_reason = (
            ...
            f"1h ADX<25确认震荡市 | 4h ADX={adx4h:.1f}<35过滤通过\n"
    ...
    # v2.24: 1h ADX >20且<35 + 1h +DI > -DI（涨力>跌力）
    if r4h['bullish'] and rd['bullish'] and pctb <= 0.15 and adx1h > 20 and adx1h < 35 and r1h['adx_pos'] > r1h['adx_neg']:
        if rsi5m >= 30 and rsi5m <= 40:
            ...
            entry_reason = (
                ...
                f"5m RSI={rsi5m:.1f} 回调到位(30~40区间)\n"
                f"4h ADX={adx4h:.1f}<35 | 1h ADX={adx1h:.1f}(20~35) +DI>{'-DI' if r1h['adx_pos'] > r1h['adx_neg'] else '+DI'} | 放量({vol_ratio:.1f}x)\n"
                f"固定止盈止损(百分比)\n"
                f"入场: ${price:,.2f}\n"
                f"止损: ${sl:,.2f} (-{STOP_LOSS_PCT*100:.1f}%)\n"
                f"止盈: ${tp1:,.2f} (+{TAKE_PROFIT_PCT*100:.1f}%) 全仓一次性"
    ...
    # v2.20: 新增 1h ADX < 30
    # v2.18: 4h ADX<35 已做全局过滤，此处仅保留信号特有条件
    # v2.0优化: 同时要求RSI>=MIN_RSI_SHORT(82)，不只是%b>0.85
    if r4h['bullish'] and rd['bullish'] and pctb > 0.85 and rsi5m >= MIN_RSI_SHORT and adx1h < 30:
        bb_u = r5m['bb_u']
        ...
        entry_reason = (
            ...
            f"4h ADX={adx4h:.1f}<35 | 1h ADX={adx1h:.1f}<30 | 放量({vol_ratio:.1f}x)\n"
            f"固定止盈止损(百分比)\n"
            f"入场: ${price:,.2f}\n"
            f"止损: ${sl:,.2f} (+{STOP_LOSS_PCT*100:.1f}%)\n"
            f"止盈: ${tp1:,.2f} (-{TAKE_PROFIT_PCT*100:.1f}%) 全仓一次性"
    ...
    # === 做空分析：大周期空头 + 5m反弹压力（逆势信号）===
    # v2.24: 1h ADX >20且<35 + 1h -DI > +DI（跌力>涨力）+ RSI 65~85
    if not r4h['bullish'] and not rd['bullish'] and pctb > 0.85 and rsi5m >= 65 and rsi5m <= 85 and adx1h > 20 and adx1h < 35 and r1h['adx_neg'] > r1h['adx_pos']:
        ...
        entry_reason = (
            ...
            f"5m %b={pctb:.3f} + RSI={rsi5m:.1f} 双超买确认(RSI 65~85)\n"
            f"4h ADX={adx4h:.1f}<35 | 1h ADX={adx1h:.1f}(20~35) -DI>{'+DI' if r1h['adx_neg'] > r1h['adx_pos'] else ''} | 放量({vol_ratio:.1f}x)\n"
            f"固定止盈止损(百分比)\n"
            f"固定止盈止损(百分比)\n"
            f"入场: ${price:,.2f}\n"
            f"止损: ${sl:,.2f} (+{STOP_LOSS_PCT*100:.1f}%)\n"
...
def open_position(direction, entry_price, atr, reason, qty, adx_1h=None, adx_4h=None):
    """v2.17 开仓 + 补仓ADX保护 + 补仓时撤销旧SL/TP，以新均价重新挂单"""
    positionSide = 'LONG' if direction == 'long' else 'SHORT'
    close_side = 'sell' if direction == 'long' else 'buy'
    ...
    # ========== v2.17: 补仓ADX安全保护 ==========
    if exchange_pos and adx_1h is not None and adx_4h is not None:
        # 补仓时：验证当前ADX是否 ≤ 首仓ADX（趋势强度必须减弱才允许补仓）
        state = load_state()
        bot_positions = state.get('positions', [])
        if bot_positions:
            first_adx_1h = bot_positions[0].get('first_entry_adx_1h')
            first_adx_4h = bot_positions[0].get('first_entry_adx_4h')
            # 向后兼容：旧仓位没有ADX记录则跳过检查
            if first_adx_1h is not None and first_adx_4h is not None:
                adx_reject_reasons = []
                if adx_1h > first_adx_1h:
                    adx_reject_reasons.append(f"1h ADX={adx_1h:.1f} > 首仓={first_adx_1h:.1f}")
                if adx_4h > first_adx_4h:
                    adx_reject_reasons.append(f"4h ADX={adx_4h:.1f} > 首仓={first_adx_4h:.1f}")
                if adx_reject_reasons:
                    reason_str = " | ".join(adx_reject_reasons)
                    log(f"⛔ 补仓ADX保护拒绝: {reason_str}")
                    log(f"   首仓ADX: 1h={first_adx_1h:.1f} 4h={first_adx_4h:.1f} | 当前ADX: 1h={adx_1h:.1f} 4h={adx_4h:.1f}")
                    return None
                else:
                    log(f"✅ 补仓ADX验证通过: 1h {adx_1h:.1f}≤{first_adx_1h:.1f} | 4h {adx_4h:.1f}≤{first_adx_4h:.1f}")
    ...
    if exchange_pos:
        ...
        # 更新state：合并为单仓位（保留首仓ADX记录）
        state = load_state()
        old_positions = state.get('positions', [])
        # v2.17: 保留首仓ADX值
        saved_adx_1h = old_positions[0].get('first_entry_adx_1h') if old_positions else adx_1h
        saved_adx_4h = old_positions[0].get('first_entry_adx_4h') if old_positions else adx_4h
        ...
        state['positions'] = [{
            ...
            'first_entry_adx_1h': saved_adx_1h,  # v2.17: 保留首仓ADX
            'first_entry_adx_4h': saved_adx_4h,
        }]
    ...
    pos_entry = {
        ...
        'open_time': datetime.now().isoformat(),
        'notified': True,  # 开仓即通知，健康检查不再补发
        'first_entry_adx_1h': adx_1h,  # v2.17: 记录首仓1h ADX
        'first_entry_adx_4h': adx_4h,  # v2.17: 记录首仓4h ADX
    }
...
# ========== 主循环 ==========
def main():
    log(f"🚀 BTC自动交易启动 v2.24 | {POLL_INTERVAL}秒轮询 | {LEVERAGE}x | 首仓{INITIAL_QTY}+补仓{ADDON_QTY}BTC | 上限{MAX_POSITIONS_PER_DIR}仓")
    log(f"v2.24: 信号3 +DI>-DI方向惯性 | 信号6 -DI>+DI方向惯性")
