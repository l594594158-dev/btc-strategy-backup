...
# 合约客户端 — 下单/仓位/K线（HYPE无现货，统一用合约K线）
...
SYMBOL = 'HYPE/USDT:USDT'       # 合约交易对 (下单用)
SYMBOL_KLINE = 'HYPE/USDT:USDT'  # K线来源 (合约，HYPE无现货)
...
# ========== 文件路径 ==========
STATE_FILE = '/root/.openclaw/workspace/hype-strategy-task/databases/state.json'
NOTIFY_QUEUE = '/root/.openclaw/workspace/hype-strategy-task/databases/notify_queue.json'
WORK_LOG = '/root/.openclaw/workspace/hype-strategy-task/logs/work_log.txt'
...
def get_data():
    """
    HYPE无现货，统一使用合约K线 (fapi.binance.com)
    """
    k5m = binance.fetch_ohlcv(SYMBOL_KLINE, timeframe='5m', limit=LIMIT_5M)
    k1h = binance.fetch_ohlcv(SYMBOL_KLINE, timeframe='1h', limit=LIMIT_1H)
    k4h = binance.fetch_ohlcv(SYMBOL_KLINE, timeframe='4h', limit=LIMIT_4H)
    k1d = binance.fetch_ohlcv(SYMBOL_KLINE, timeframe='1d', limit=LIMIT_1D)
...
def check_entry(r5, r1, r4, rd):
    ...
    if abs(pct_sma) > 1.5:
        return None, f"观望 | 价距5mSMA {pct_sma:+.1f}% (需±1.5%)"
...
def do_open(direction, reason):
    ...
    try:
        ...
        if has_pos:
            ...
            if not state.get(pos_key):
                for p in positions:
                    if p.get('symbol') == SYMBOL and p.get('side') == side_check:
                        entry = float(p.get('entryPrice', 0))
                        amt = abs(float(p.get('contracts', 0) or 0))
                        sl = round(entry * (1 - SL_PCT if direction == 'LONG' else 1 + SL_PCT), 4)
                        tp = round(entry * (1 + TP_PCT if direction == 'LONG' else 1 - TP_PCT), 4)
                        state[pos_key] = {
                            'entry_price': entry, 'qty': amt, 'sl': sl, 'tp': tp,
                            'open_time': datetime.now().strftime('%Y-%m-%dT%H:%M:%S'),
                            'reason': {'name': f'{direction}-交易所同步', 'price': entry}
                        }
                        save_state(state)
                        log(f"🔄 交易所{direction} {amt}HYPE @ ${entry:,.4f} → 已同步state")
                        break
            return False
    except Exception as e:
        log(f"❌ 交易所检查失败: {e}")
    ...
    log(f"🚨 开{direction} | ${price:,.4f} | SL=${sl_price:,.4f} | TP=${tp_price:,.4f}")
    ...
    try:
        ...
        log(f"✅ 已开仓: {direction} {QTY}HYPE @ ${fill_price:,.4f}")
    except Exception as e:
        log(f"❌ 开仓失败: {e}")
        return False
    ...
    reason_text = (
        f"【{reason['name']}】\n"
        f"价格: ${reason['price']:,.4f} | SMA5: ${reason['sma5']:,.4f} ({reason['pct_sma']:+.1f}%)\n"
        f"RSI5: {reason['rsi5']:.1f} | 1hADX: {reason['adx_1h']:.1f} | 4hADX: {reason['adx_4h']:.1f}\n"
        f"4h闭K: ${reason['close_4h']:,.4f} vs SMA${reason['sma_4h']:,.4f} | "
        f"1d闭K: ${reason['close_1d']:,.4f} vs SMA${reason['sma_1d']:,.4f}\n"
    ...
    wechat_msg = (
        ...
        f"开仓价: ${fill_price:,.4f}\n"
        f"止损: ${sl_price:,.4f} ({'-'}{SL_PCT*100:.1f}%)\n"
    ...
    work_log('开仓', f'{direction} {QTY}HYPE @ {fill_price} | SL={sl_price} TP={tp_price}')
    return True
...
def manage_positions(state):
    """
    主动出场管理 (每2秒):
    LONG:  pnl ≤ -2% 止损 / pnl ≥ +3% 止盈
    SHORT: pnl ≤ -2% 止损 / pnl ≥ +3% 止盈
    """
    try:
        positions = binance.fetch_positions()
    except:
    ...
    for pos_key, direction in [('long_pos', 'LONG'), ('short_pos', 'SHORT')]:
        ...
        if pnl_pct <= -SL_PCT * 100:
            log(f"🛑 {direction}触发主动止损 | pnl={pnl_pct:+.2f}% | 标记价=${mark_price:,.4f}")
            try:
                close_side = 'sell' if direction == 'LONG' else 'buy'
                binance.create_order(
                ...
                send_wechat(
                    f"🛑 HYPE v4.2 主动止损\n"
                    f"{direction} {qty}HYPE @ ${entry:,.4f}\n"
                    f"触发价: ${mark_price:,.4f} | pnl: {pnl_pct:+.2f}%\n"
                    f"⏰ {datetime.now().strftime('%H:%M:%S')}"
                )
                work_log('主动止损', f'{direction} {qty}HYPE @ {entry} | pnl={pnl_pct:+.2f}%')
                state[pos_key] = None
        ...
        # === 止盈 ===
            ...
            log(f"🎯 {direction}触发主动止盈 | pnl={pnl_pct:+.2f}% | 标记价=${mark_price:,.4f}")
            try:
                close_side = 'sell' if direction == 'LONG' else 'buy'
                binance.create_order(
                    SYMBOL, 'market', close_side, qty,
                ...
                work_log('主动止盈', f'{direction} {qty}HYPE @ {entry} | pnl={pnl_pct:+.2f}%')
                state[pos_key] = None
                state[f'last_{direction.lower()}_signal'] = False
                changed = True
...
def check_close(state):
    ...
    if state.get('long_pos') and not long_alive:
        pos = state['long_pos']
        log(f"📤 LONG已平仓 | 开仓=${pos['entry_price']:,.4f}")
        send_wechat(f"✅ HYPE v4.2 平仓\nLONG {pos['qty']}HYPE @ ${pos['entry_price']:,.4f}\n⏰ {datetime.now().strftime('%H:%M:%S')}")
        work_log('平仓', f"LONG {pos['qty']}HYPE @ {pos['entry_price']}")
        state['long_pos'] = None
    ...
    if state.get('short_pos') and not short_alive:
        pos = state['short_pos']
        log(f"📤 SHORT已平仓 | 开仓=${pos['entry_price']:,.4f}")
        send_wechat(f"✅ HYPE v4.2 平仓\nSHORT {pos['qty']}HYPE @ ${pos['entry_price']:,.4f}\n⏰ {datetime.now().strftime('%H:%M:%S')}")
        work_log('平仓', f"SHORT {pos['qty']}HYPE @ {pos['entry_price']}")
...
def print_status(r5, r1, r4, rd, state, signal, reason):
    ...
    pos_str = ''
    if long_pos: pos_str += f'🟢L {long_pos["qty"]}HYPE @ ${long_pos["entry_price"]:,.4f} '
    if short_pos: pos_str += f'🔴S {short_pos["qty"]}HYPE @ ${short_pos["entry_price"]:,.4f} '
    ...
    print(f"""
╔════════════════════════════════════════════╗
║  HYPE v4.2 趋势回调  |  {pos_str}
╠════════════════════════════════════════════╣
║  💰 ${price:>12,.4f}  | 距SMA5 {pct_sma:+.1f}%  | RSI {rsi5:.1f}         ║
║  4h闭K: ${c4h:,.4f} vs SMA${s4h:,.4f} → {trend4}头          ║
║  1d闭K: ${c1d:,.4f} vs SMA${s1d:,.4f} → {trend1}头          ║
║  1hADX: {adx_1h:.1f}  | 4hADX: {adx_4h:.1f}  | 量比: {vol5:.1f}x           ║
║  信号: {sig_str}  | {obs_str} ║
╚════════════════════════════════════════════╝""")
    # 简洁版日志
    log(f"${price:,.4f} | 5mSMA{sma5:,.4f}({pct_sma:+.1f}%) | RSI{rsi5:.1f} | 1hADX{adx_1h:.1f} 4hADX{adx_4h:.1f} | 量{vol5:.1f}x | 4h闭{c4h:,.4f}/SMA{s4h:,.4f}| 1d闭{c1d:,.4f}/SMA{s1d:,.4f} | {sig_str}")
...
def main():
    ...
    log(f"LONG: 4h多+1d多(闭K) + 5mSMA±1.5% + 1hADX>25(闭) + 4hADX<40(闭) + RSI>40 + 量≥1.0(闭)")
    log(f"SHORT: 4h空+1d空(闭K) + 5mSMA±1.5% + 1hADX>25(闭) + 4hADX<40(闭) + RSI<60 + 量≥1.0(闭)")
    ...
    try:
        ...
        for p in positions:
            ...
            if amt > 0 and p.get('symbol') == SYMBOL:
                ...
                entry = float(p.get('entryPrice', 0))
                sl = round(entry * (1 - SL_PCT if d == 'LONG' else 1 + SL_PCT), 4)
                tp = round(entry * (1 + TP_PCT if d == 'LONG' else 1 - TP_PCT), 4)
                if not state.get(pk):
                    state[pk] = {
                        'entry_price': entry, 'qty': amt, 'sl': sl, 'tp': tp,
                        'open_time': datetime.now().strftime('%Y-%m-%dT%H:%M:%S'),
                        'reason': {'name': f'{d}-启动同步', 'price': entry}
                    }
                    log(f"🔄 启动同步交易所{d} {amt}HYPE @ ${entry:,.4f} → state")
        save_state(state)
    except Exception as e:
        log(f"⚠️ 启动同步异常: {e}")
